...
class TransformRotate(object):
    def __init__(self, mean, std, crop_size):
        ...
        self.rotate_90 = transforms.RandomRotation((90, 90))
        self.rotate_180 = transforms.RandomRotation((180, 180))
        self.rotate_270 = transforms.RandomRotation((270, 270))
    ...
    def __call__(self, x):
        ...
        x_90 = self.rotate_90(self.base_transform(x))
        x_180 = self.rotate_180(self.base_transform(x))
        x_270 = self.rotate_270(self.base_transform(x))
        return [self.normalize(x_0),
               self.normalize(x_90),
               self.normalize(x_180),
               self.normalize(x_270)]


class TransformRotateFix(object):
    def __init__(self, mean, std, crop_size):
        self.base_transform = transforms.Compose([
            ResizeImage(256),
            transforms.RandomHorizontalFlip(),
    ...
    def __call__(self, x):
        ...
        x_0_strong = self.strong_transform(x)
        return (
                   [self.normalize(x_0),
                   self.normalize(x_90),
                   self.normalize(x_180),
...
def return_dataset_s4l_fixmatch(args):
    ...
    target_dataset_val = Imagelists_VISDA(image_set_file_t_val, root=root,
                                          transform=data_transforms['val'])
    target_dataset_unl = Imagelists_VISDA(image_set_file_unl, root=root,
                                          transform=data_transforms['train_ut'], rotate=True)
    target_dataset_test = Imagelists_VISDA(image_set_file_unl, root=root,
                                           transform=data_transforms['test'])
    class_list = return_classlist(image_set_file_s)
    print("%d classes in this dataset" % len(class_list))
...
def return_dataset_s4l_fixmatch_mixup(args):
    base_path = './data/txt/%s' % args.dataset
    root = './data/%s/' % args.dataset
    image_set_file_s = \
        os.path.join(base_path,
    ...
    if args.net == 'alexnet':
        ...
        crop_size = 224
    data_transforms = {
        'train_s': TransformRotateFix(mean=[0.485, 0.456, 0.406],
                                      std=[0.229, 0.224, 0.225],
                                      crop_size=crop_size),
        'train_ut': TransformRotateFix(mean=[0.485, 0.456, 0.406],
                                       std=[0.229, 0.224, 0.225],
...
def return_dataset_fixmatch(args):
    base_path = './data/txt/%s' % args.dataset
    root = './data/%s/' % args.dataset
    image_set_file_s = \
        os.path.join(base_path,
    ...
    data_transforms = {
        ...
        'train_ut': TransformFix(mean=[0.485, 0.456, 0.406],
                                 std=[0.229, 0.224, 0.225],
                                 crop_size=crop_size),
        'val': transforms.Compose([
            ResizeImage(256),
    ...
    source_dataset = Imagelists_VISDA(image_set_file_s, root=root,
                                      transform=data_transforms['train_s'])
    target_dataset = Imagelists_VISDA(image_set_file_t, root=root,
                                      transform=data_transforms['val'])
    target_dataset_val = Imagelists_VISDA(image_set_file_t_val, root=root,
                                          transform=data_transforms['val'])
