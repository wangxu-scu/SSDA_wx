...
from utils.return_dataset import return_dataset_s4l_fixmatch
...
parser.add_argument('--exp_name', type=str, default='2nd_5',
...
source_loader, target_loader, target_loader_unl, target_loader_val, \
    target_loader_test, class_list = return_dataset_s4l_fixmatch(args)
...
def train():
    ...
    for step in range(all_step):
        ...
        ## Unsupervised Loss
        output = G(torch.cat((im_data_tu, im_data_tu_strong), 0))
        output_tu, output_tu_strong = output[:len(im_data_tu)], output[len(im_data_tu):]
        ...
        loss_u = (F.cross_entropy(logits_tu_strong, targets_tu, reduction='none') * mask).mean()
